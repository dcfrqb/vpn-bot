"""
Система ручной модерации платежей.
Заявки хранятся в тексте сообщения админу (#PAYREQ блок) с HMAC-подписью.
"""
import hashlib
import hmac
import re
...
def verify_signature(v: str, req_id: str, tg_id: int, tariff: str, amount: int, currency: str, created: str, sig: str) -> bool:
...
class PaymentRequestData:
    ...
    def to_sign_string(self) -> str:
        return f"{self.req_id}|{self.tg_id}|{self.tariff}|{self.amount}|{self.currency}|{self.created}"
