...
async def test_already_used_rejects_solokhin():
    ...
    with patch("app.routers.start.SyncService") as MockSync, \
         patch("app.routers.start.settings") as mock_settings, \
         patch("app.routers.start.SessionLocal", return_value=_existing_payment_session()):
        mock_settings.ADMINS = []
        sync_inst = MagicMock()
...
async def test_solokhin_grants_premium_15d():
    ...
    with patch("app.routers.start.SyncService") as MockSync, \
         patch("app.routers.start.settings") as mock_settings, \
         patch("app.routers.start.SessionLocal", return_value=_no_payment_session()), \
         patch("app.routers.start.provision_tariff", new_callable=AsyncMock, return_value=True), \
    ...
    assert "Premium" in user_text
    assert "15" in user_text
    # Admin notification
    msg.bot.send_message.assert_called_once()
    admin_text = msg.bot.send_message.call_args[1]["text"]
    assert "SOLOKHIN" in admin_text
    assert "Premium" in admin_text
    assert "15" in admin_text
...
async def test_trial_grants_basic_10d():
    ...
    msg.answer.assert_called_once()
    user_text = msg.answer.call_args[0][0]
    assert "Basic" in user_text
    assert "10" in user_text
    msg.bot.send_message.assert_called_once()
    admin_text = msg.bot.send_message.call_args[1]["text"]
    assert "TRIAL" in admin_text
    assert "Basic" in admin_text
    assert "10" in admin_text


# ---------------------------------------------------------------------------
# user messages are different between /solokhin and /trial
# ---------------------------------------------------------------------------
...
async def test_solokhin_uses_correct_tariff():
    ...
    with patch("app.routers.start.SyncService") as MockSync, \
         patch("app.routers.start.settings") as mock_settings, \
         patch("app.routers.start.SessionLocal", return_value=_no_payment_session()), \
         patch("app.routers.start.provision_tariff", new_callable=AsyncMock, return_value=True) as mock_prov, \
